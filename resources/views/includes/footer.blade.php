   ...
   document.addEventListener("DOMContentLoaded", function() {
    ...
    searchInput.addEventListener("keyup", function() {
        ...
        if (searchQuery.length > 2) {
            ...
            xhr.open("GET", "{{ route('home.search') }}?query=" + encodeURIComponent(searchQuery), true);
...
document.addEventListener('DOMContentLoaded', function()  {
    ...
    // Handle brand checkboxes change
    document.querySelectorAll("input[name='brands']").forEach(function (checkbox) {
        checkbox.addEventListener("change", function () {
            let selectedBrands = [];
            document.querySelectorAll("input[name='brands']:checked").forEach(function (checkedBox) {
                selectedBrands.push(checkedBox.value);
            });

            document.getElementById("hdnBrands").value = selectedBrands.join(",");
            document.getElementById("frmfilter").submit();
        });
    });

    // Handle category checkboxes change
    document.querySelectorAll("input[name='categories']").forEach(function (checkbox) {
        checkbox.addEventListener("change", function () {
            ...
            document.getElementById("hdCategories").value = selectedCategories.join(",");
            document.getElementById("frmfilter").submit();
        });
    });

    // Handle price range change
    document.querySelector("input[name='price_range']").addEventListener("change", function () {
        let [min, max] = this.value.split(',');

        document.getElementById("hdMinPrice").value = min;
        document.getElementById("hdMaxPrice").value = max;
