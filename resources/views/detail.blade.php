      ...
      <div class="col-lg-5">
        ...
        <div class="product-single__price">
          <span class="current-price">
            @if($product->sale_price)
                    <s class="p-2">${{ $product->regular_price }}</s>${{ $product->sale_price }}
                    @else
        ...
        <div class="product-single__addtolinks">
          ...
          <form action="{{ route('wishlist.remove',['rowId'=>Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId]) }}" method="POST" id="frm-remove-item">
            @csrf
            @method('DELETE')
              <a href="javascript:viod(0)" class="text-danger fw-bolder menu-link menu-link_us-s add-to-wishlist" onclick="document.getElementById('frm-remove-item').submit();"><svg width="16" height="16" viewBox="0 0 20 20"
            ...
            <form method="POST" action="{{ route('wishlist.add') }}" id="wishlist-form">
              ...
              <input type="hidden" name="id" value="{{ $product->id }}">
              <input type="hidden" name="name" value="{{ $product->name }}">
              <input type="hidden" name="price" value="{{ $product->sale_price == '' ? $prodect->regular_price : $product->sale_price }}">
              <input type="hidden" name="quantity" value="1"/>
            <a href="javascript:viod(0)" class="menu-link menu-link_us-s add-to-wishlist" onclick="document.getElementById('wishlist-form').submit();"><svg width="16" height="16" viewBox="0 0 20 20"
              fill="none" xmlns="http://www.w3.org/2000/svg">
              <use href="#icon_heart" />
